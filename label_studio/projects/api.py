...
class ProjectListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        is_superadmin = self._is_super_admin(self.request.user)
        
        # Check if user is admin
        is_admin = self._is_admin(self.request.user)
        ...
        elif show_all_param or is_superadmin or is_admin:
            # Show all projects in organization (for admin/total count purposes, superadmin, or admin)
    ...
    def _is_admin(self, user):
        ...
        try:
            from users.role_models import UserRoleAssignment
            # Check for admin, super-admin, and administrator roles
            return UserRoleAssignment.objects.filter(
                user=user, 
