...
def save_user(request, next_page, user_form):
    ...
    # Check if user was invited via token
    org = getattr(request, 'validated_organization', None)
    
    if org:
        # Use the validated organization from the invite token
        org.mark_token_used()  # Mark token as used
        org.add_user(user)
    else:
        # Default behavior for users without invite token
