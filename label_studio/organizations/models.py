...
class Organization(OrganizationMixin, models.Model):
    ...
    token_created_at = models.DateTimeField(_('token created at'), null=True, blank=True)
    token_expires_at = models.DateTimeField(_('token expires at'), null=True, blank=True)
    token_used_at = models.DateTimeField(_('token used at'), null=True, blank=True)
    ...
    def reset_token(self):
        ...
        self.token_expires_at = timezone.now() + timedelta(days=7)  # Token expires in 7 days
        self.token_used_at = None  # Reset usage tracking
        self.save(update_fields=['token', 'token_created_at', 'token_expires_at', 'token_used_at'])

    def is_token_valid(self, token):
        """Check if the provided token is valid for this organization"""
        if not self.token or self.token != token:
            return False
        if self.token_expires_at and timezone.now() > self.token_expires_at:
            return False
        if self.token_used_at:
    ...
    @classmethod
    def get_by_valid_token(cls, token):
        """Get organization by valid token"""
        try:
            org = cls.objects.get(token=token)
            if org.is_token_valid(token):
                return org
