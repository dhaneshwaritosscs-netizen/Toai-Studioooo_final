...
const { EnvironmentPlugin, DefinePlugin, ProgressPlugin, optimize, ProvidePlugin } = require("webpack");
...
const DJANGO_HOSTNAME = process.env.DJANGO_HOSTNAME || "http://localhost:8083";
...
const plugins = [
  ...
  // Provide jQuery globally for compatibility with Django templates
  new ProvidePlugin({
    $: 'jquery',
    jQuery: 'jquery',
    ...
    return merge(config, {
      ...
      devServer: process.env.MODE?.startsWith("standalone")
        ...
        : {
            ...
            // Use auto for allowedHosts in webpack-dev-server 5.x
            allowedHosts: "auto",
            proxy: {
              "/api": {
